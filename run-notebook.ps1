...
$volumeMounts = "--mount type=bind,source='${scriptPath}',target=/home/spendyala/project"
if (Test-Path -Path ("${env:userprofile}\.aws")) {
    Write-Host "We have a $HOME/.aws directory, mounting" -ForegroundColor Green
    $volumeMounts = "${volumeMounts} --mount type=bind,source='${env:userprofile}\.aws',target=/home/spendyala/.aws".Trim()
}

$dockerEnv = "${dockerEnv} -e NOTEBOOK_MODE=lab".Trim()
$runCommand = "docker run --rm -it -p 9999:9999 --init --name '${name}' ${dockerEnv} $volumeMounts $image"

if (-not $NoBrowser) {
    Start-Job {
        Start-Sleep 2
        Write-Host "`n*** Opening Browser to http://localhost:9999/lab ***`n" -ForegroundColor Green
        Start-Process http://localhost:9999/lab
    }
}
else
{
